...
- 결과 csv 컬럼: round, sim_time, **round_duration**, **round_duration_greedy**,
  ...
  - `tau` — D 에서 추가. tau 를 sweep 하는 실험이 있으므로 컬럼으로 들고 있어야 한다
    (config 기본값으로 되돌려 읽으면 sweep 행을 구분할 수 없다).
...
- **선택 수 불변식**: 선택 단말 수 == `sum_j min(M, |C_j 안의 후보 수|)`.
  **C 단계 정정**: `min(M×J, K, tau 통과 수)` 는 **상한일 뿐 등호가 아니다.**
  후보가 클러스터에 고르게 퍼지지 않으면(한 클러스터에 M 개 넘게 몰리면) 못 채운다.
  실측(alpha=0.05, K=50, M=2, 시드3 x 호버링3): tau 통과 9.9 / 실제 선택 **8.2** 대.
  논문 Table 5 의 "평균 선택 단말 수 = M×J" 는 후보가 고르게 퍼질 때만 성립한다.
  (B 단계 기록의 9.3 은 선택 수가 아니라 **같은 라운드 길이에 태울 수 있는 수**였다.
  두 값은 다른 양이다. 섞지 말 것.)
...
- **영구 배제 검증**: 충분한 라운드 후 한 번도 후보에 오르지 못한 단말 수가
  ...
  **D 실측: 19~23/50 (38~46%).** 소수이긴 하나 크다. 버그가 아니라 기하의 결과다 —
  tau=0.15 의 커버 수평반경이 89 m 라 반경 밖은 그 지점에서 t_comm >= tau 이고,
  L=3 개 원이 400x400 영역에서 덮는 면적이 최대 47% 다. tau 나 L 을 키우지 않는 한
  이 값은 내려가지 않는다. **이 수치를 보고 partition/채널 버그를 찾지 말 것.**
...
- 특정 스텝을 구현하거나 중간에 무언가를 수정할 때는, 무엇을 어떻게 한 것인지
  사용자가 쉽게 이해할 수 있도록 **답변 끝에 첨언**할 것.

## 세션 운영 — 매 스텝을 새 세션에서 시작한다
사용자는 스텝마다 새 채팅을 연다. 세션은 기억을 이어받지 못하므로 **문서 3개가 기억을
대신한다.** 역할이 다르니 섞지 말 것.

| 파일 | 역할 | 수명 |
|---|---|---|
| `CLAUDE.md` | 프로젝트 규칙과 **확정된 설계 결정**. 항상 최신 | 영구 |
| `NEXT_STEP.md` | **다음 한 스텝**의 인수인계 사양 | 그 스텝이 끝나면 다음 스텝용으로 새로 씀 |
| `GenFL-UAV_작업기록.docx` | 사람이 읽는 **작업 기록·설명·질문답변**. 계속 누적 | 영구(로컬 전용) |

### 스텝을 마칠 때 반드시 하는 4가지 (하나라도 빠뜨리면 다음 세션이 헤맨다)
1. **CLAUDE.md 갱신** — 진행 현황 체크리스트, 새로 확정된 설계 결정, 재측정 불필요한 수치.
2. **작업기록.docx 이어쓰기** — 아래 절차대로. 새로 만들지 말고 **기존 파일에 추가**한다.
3. **NEXT_STEP.md 를 다음 스텝용으로 새로 쓴다** — 기존 내용을 지우고 다음 스텝 사양으로
   교체한다. 반드시 포함할 것: 시작 전 확인 명령, 만들 파일과 사양, 이미 측정된 값,
   **이번 스텝에서 실제로 틀렸던 것 목록**, 완료 기준.
4. **커밋** — 위 문서 변경까지 함께.

작업기록.docx 는 `.gitignore` 로 제외된 **로컬 전용** 파일이다. 깃에 올리지 않는다.
clone 한 환경(Kaggle 등)에는 없으므로, 거기서는 이 파일을 찾지 말 것.

### 작업기록.docx 이어쓰는 법
`python-docx` 로 기존 파일을 열어 추가한다. 문서 0장의 "갱신 규칙"에 같은 내용이 있다.
...
doc = docx.Document(PATH)                      # 기존 문서를 연다
doc.add_page_break()
doc.add_heading("2.3 Step C — 클러스터링과 단말 선택", level=2)
doc.add_paragraph("한 일 ...")
doc.save(PATH)
```
- 표는 `style="Light Grid Accent 1"`, 글꼴은 맑은 고딕 9pt 로 기존과 맞춘다.
- 2장에 「Step X」 절 추가 (구성: 한 일 / 설계 결정과 이유 / 측정 결과).
- 사용자가 개념을 물어봤으면 3장에 Q&A 추가.
- **틀렸다가 바로잡은 것은 4장 표에 반드시 한 줄 추가한다. 숨기지 않는다.**
...
- [x] B: `channel.py`(fixed/literal 2모드), `device.py`, `hovering.py`(K-means 직접 구현),
  ...
  **B 에서 실측으로 확인한 것 (다음 세션에서 재계산 불필요):**
  ...
  4. ~~**배터리 필터는 실제로 작동한다.** alpha=0.05 seed1 에서 2개 단말이 800 라운드
     내 고갈.~~ → **D 에서 반증됐다. 800 라운드 안에 고갈되는 단말은 0 대다.**
     이 추정은 "매 라운드 참여"를 가정했는데, 호버링 순회 때문에 한 단말은 L=3 지점 중
     자기를 덮는 지점의 라운드에만 참여한다. 실측 최대 참여 268/800회(정확히 1/3),
     최대 예산 소진율 34~45%. **배터리 필터는 800 라운드 설정에서 구속력이 없다.**
     라운드 수를 2,000 이상으로 늘리거나 L 을 줄이면 그때 구속하기 시작한다.
...
- [x] C: `network/clustering.py`(Eq.4,5), `scheduler.pipelined_round_time`,
      `fl/selection.py`(Eq.8 + 필터 2개 + 워밍업), `tests/test_clustering.py`,
      `tests/test_selection.py`. **테스트 6파일 51개 전부 통과** (B 시점 30개 → +21).

  **C 에서 실측으로 확인한 것 (재계산 불필요):**
  1. **J = 4 / 6 / 5** (seed 0/1/2, tau=0.15). J·tau = 0.60/0.90/0.75 이고
     spread = 0.685/0.921/0.820 이라 `J·tau ≤ spread < (J+1)·tau` 가 세 시드 모두 성립.
  2. **theta_J == max t_comp 가 세 시드 모두 정확히 성립한다.** J·tau ≤ spread 가
     Eq.(4) 의 정의상 보장하므로 데드라인 누적이 max t_comp 를 넘지 못한다.
     → **이 설정에서 파이프라인은 라운드를 늘리지 않는다.** 다만 J 를 강제로 키우거나
     tau 를 키우면 깨질 수 있는 성질이므로 M/tau sweep 에서 다시 확인한다.
  3. **참여 규모 주장은 성립한다.** 같은 라운드 길이
     (= max t_comp + 업로드 1회, 논문 Fig.2 의 conventional 라운드) 안에서
     **sync 는 정확히 2.0 대(=M), pipelined 는 9.3 대**를 태운다. 9개 조합 전부.
  4. **그런데 선택된 M×J 대만 놓고 재면 파이프라인이 항상 빠르지는 않다.**
     pipe/sync = 평균 0.896, 범위 0.689~1.480, **9개 중 2개에서 1을 넘었다.**
     원인: sync 는 *선택된* 단말의 max t_comp 만 기다리면 되는데, theta 는 *전체 K* 로
     계산된 값이라 빠른 클러스터의 선택 단말이 자기 theta_j 까지 억지로 기다린다.
     전 K 대가 업로드하는 경우에는 0.918~0.940 으로 항상 파이프라인이 빠르다.
  5. **greedy 가 pipelined 보다 9개 중 7개에서 빨랐다 (평균 0.769, 나머지 2개는 동률).**
     클러스터링·tau·t_comp 사전수집이라는 조율 장치 없이 "끝나는 대로 빈 채널"만으로
     같은 중첩이 나온다는 뜻이다. CLAUDE.md "greedy 스케줄러 대조" 가 예고한 결과이고,
     **논문이 검증하지 않은 지점이므로 그대로 보고한다.** F 단계에서 학습과 함께 재확인.
  6. 워밍업(r=1)은 **필터를 무시하고 전원 참여**시킨다. 필터를 걸면 걸린 단말의
     r' 과 초기 손실이 영영 비어 Eq.(8) 을 계산할 수 없기 때문이다.
- [x] D: **검증 게이트 통과.** `experiments/exp1_gate_table5.py` (학습 없는 라운드 루프),
      `tests/test_gate.py`. **테스트 7파일 59개 전부 통과** (C 시점 51개 → +8).
      결과: `results/exp1_gate_table5.csv` (48 run = 2방법 x tau 4 x M 2 x 시드 3,
      800라운드, 38,400행), 판정 출력 `results/exp1_gate_report.txt`. 전체 9초.

  **게이트 6항목 전부 통과.** 재측정 불필요한 수치:
  1. `J·tau ≤ spread < (J+1)·tau` — 48 run 전부 성립 (위반 0).
  2. **tau ↓ → J ↑ 단조** — tau [0.10/0.12/0.15/0.20] 에서
     seed0 6/5/4/3, seed1 9/7/6/4, seed2 8/6/5/4.
  3. **선택 수 불변식 위반 0 / 38,400 라운드.** 등호는 `sum_j min(M, 클러스터 후보 수)`
     이고 M×J 가 아니다 (C 에서 정정한 것이 800라운드 규모에서 확인됐다).
  4. 영구 배제 19~23/50 — 기하의 결과다 (위 "영구 배제 검증" 항목 참조).
  5. 배터리 고갈 **0 대** — B 의 "2대 고갈" 추정 반증 (위 B 절 4번 참조).
  6. `round_duration` / `round_duration_greedy` 38,400행 전부 채워짐.

  **논문 Table 5 구조 재현 (M=2, 시드 3개 x 800라운드 평균):**

  | 우리 tau | J | 평균 선택 | | 논문 tau | J | 평균 선택 |
  |---|---|---|---|---|---|---|
  | 0.10 | 7.7 | **0.0** | | 0.06 | 16 | **0** |
  | 0.12 | 6.0 | 3.0 | | 0.08 | 12 | 6 |
  | 0.15 | 5.0 | **8.2** | | 0.10 | 10 | **20** |
  | 0.20 | 3.7 | 7.0 | | 0.12 | 8 | 16 |

  숫자가 아니라 **모양이 같다**: tau 를 키우면 필터는 느슨해지지만 J 가 줄어 참여가
  정점을 찍고 꺾인다. 정점 위치도 양쪽 다 세 번째 칸이다. tau=0.15 의 8.2 는 C 의
  실측 8.2 와 정확히 일치한다 (라운드 루프가 단발 측정과 어긋나지 않는다는 확인).

  **라운드 길이 — greedy 대조 (tau=0.15, 시드 3개 x 800라운드):**

  | method | M | 기준[s] | greedy[s] | greedy/기준 |
  |---|---|---|---|---|
  | FedAvg | 2 | 0.351 | 0.344 | 0.982 |
  | FedAvg | 4 | 0.481 | 0.469 | 0.975 |
  | PUFL | 2 | 0.915 | 0.708 | **0.774** |
  | PUFL | 4 | 0.917 | 0.688 | **0.750** |

  → **greedy 가 파이프라인보다 23~25% 빠르다** (C 의 단발 측정 0.769 와 일치).
  조율 장치(클러스터링·tau·t_comp 사전수집) 없이 "끝나는 대로 빈 채널"만으로 같은
  중첩이 나온다. F 에서 학습과 함께 재확인한 뒤 그대로 보고한다.
  → PUFL 의 라운드 길이가 **M 에 거의 무관하다** (0.915 → 0.917). M 이 채널 수와
  클러스터당 선택 수를 동시에 뜻해 두 효과가 상쇄되기 때문이다 (불변 조건에 예고된 대로).
